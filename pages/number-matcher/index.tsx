import {
  ...
  MatchingRow,
  SantisedInvoice,
} from "@/containers/number-matcher/types";
import {
  downloadCsvFromUri,
  highlightCsv,
  readCsv,
  searchAgent,
} from "@/containers/number-matcher/utils";
...
export const NumberMatcher = () => {
  ...
  const run = async () => {
    ...
    const operaInvoiceData = await readCsv(
      operaFile.current,
      operaCardNumberHeader.trim(),
      operaAmountHeader.trim()
    );
    console.log({ operaInvoiceData });
    const agentInvoiceData = await readCsv(
      agentFile.current,
      cardNumberHeader.trim(),
      amountHeader.trim()
    );
    ...
    for (let i = 0; i < operaInvoiceData.length; i++) {
      ...
      const matchingAgentIndex = searchAgent(
        operaRow,
        agentInvoiceData,
        duplicates
      );
      const key = `${operaRow.last4Digits}-${operaRow.amount}`;

      if (matchingAgentIndex > -1) {
        if (duplicates[key]) {
          duplicates[key].push(matchingAgentIndex);
        } else {
          duplicates[key] = [matchingAgentIndex];
        }
  ...
  const agentSortedMatchingRows = matchingRows.sort(
    (a, b) => a.agentIndex - b.agentIndex
  );
        ...
        <div className="mb-4">
          ...
          <div className="mb-4">
            ...
            <input
              ...
              type="text"
              onChange={(e) => {
                setCardNumberHeader(e.target.value);
              }}
          ...
          <div className="mb-4">
            ...
            <input
              ...
              type="text"
              onChange={(e) => {
                setAmountHeader(e.target.value);
              }}
              value={amountHeader}
        ...
        {matchingRows.length > 0 && (
          <div>
            ...
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                {agentSortedMatchingRows.map(
                  ({ agentIndex, operaIndex }, index) => {
                    ...
                    return (
                      <tr
                        key={agentIndex + operaIndex + operaRow.last4Digits}
                        className={`bg-white border-b dark:bg-gray-800 dark:border-gray-700 ${
                          isDuplicate ? "text-red-400" : ""
                        }`}
                      >
